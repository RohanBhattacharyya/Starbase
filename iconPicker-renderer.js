document.addEventListener('DOMContentLoaded', async () => {
    ...
    const fontAwesomeIcons = [
        ...
        'fa-solid fa-apple-whole', 'fa-solid fa-lemon', 'fa-solid fa-seedling', 'fa-solid fa-mouse',
        'fa-solid fa-hospital', 'fa-solid fa-microscope', 'fa-solid fa-dna', 'fa-solid fa-atom',
        'fa-solid fa-virus', 'fa-solid fa-bacteria', 'fa-solid fa-disease',
    ];

    async function loadCustomIcons() {
        customIcons = await window.iconPickerAPI.getCustomIcons();
    }

    function renderIcons(filter = '') {
        iconGrid.innerHTML = '';
        const allIcons = [...customIcons, ...fontAwesomeIcons];
        const filteredIcons = allIcons.filter(icon => {
            if (icon.startsWith('fa-')) {
                return icon.includes(filter.toLowerCase());
            } else {
        ...
        filteredIcons.forEach(icon => {
            ...
            iconElement.className = `icon-card ${selectedIcon === icon ? 'selected' : ''}`;
            iconElement.dataset.iconClass = icon;

            if (icon.startsWith('fa-')) {
                iconElement.innerHTML = `<i class="fas ${icon}"></i><p>${icon.replace('fa-solid fa-', '')}</p>`;
            } else {
                const filename = icon.split('/').pop().split('\\').pop();
                iconElement.innerHTML = `<img src="${icon}" alt="${filename}" class="custom-icon"><p>${filename}</p>`;
            }
    ...
    importButton.addEventListener('click', async () => {
        const newIcon = await window.iconPickerAPI.importIcon();
        if (newIcon) {
            await loadCustomIcons();
            renderIcons(iconSearchInput.value);
        }
    });
